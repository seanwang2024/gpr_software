...
Qt 6.8.3 + MinGW + OpenCV 4.11.0 static, **当前版本 v1.0.185**(已发布FTP+安装包并验证).
...
**自动增益(typeId 0x1a)第1轮(§9, 进行中)**:
- proc布局**非对齐**: `1a <点数u8> <1B> f32@3(整体增益) f32@7(水平时常)`; P_R=1/0/0, P_S=8/2.0/20.0
- **点数1已定量**(P_R): R=clamp(0.71784×S, ±2^24)+行1哨兵, corr=1.0000 纯常数缩放; P_E同款0.72014(两常数0.3%微差待释)
- **点数8形态已明**(P_S): 分段AGC — 每道深度分8段→段增益→段间插值→水平指数平滑(τ=20道)→×整体增益2.0; corr=0.893, 深部能量×11, >2^24不钳位; **待网格拟合后落地增益面板**
...
## 待做
- **自动增益落地(当前任务)**: P_S分段AGC网格拟合(段增益定义RMS/均值/包络×插值×水平平滑α) → 增益面板"自动增益"(点数/整体增益/水平时常) + proc写`1a`; 点数1常数0.71784公式来源
